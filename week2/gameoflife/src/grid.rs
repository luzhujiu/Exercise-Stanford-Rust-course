...
impl Grid {
    ...
    pub fn get(&self, row: usize, col: usize) -> Option<usize> {
        if let Some(pos) = self.location(row, col) {
            let elem = self.elems[pos];
            return Some(elem);            
        } else {
            return None;
        }
    ...
    fn location(&self, row: usize, col: usize) -> Option<usize> {
        if col >= self.num_cols || row >= self.num_rows {
            return None;
        }
        let seq = self.num_cols * row + col;
        return Some(seq);
    }
    ...
    pub fn set(&mut self, row: usize, col: usize, val: usize) -> Result<(), &'static str> {
        if let Some(pos) = self.location(row, col) {
            ...
            Err("position is out of bound.")
        }
    }

    /// Prints a visual representation of the grid. You can use this for debugging.
    pub fn display(&self) {
        println!("rows = {} cols = {}", self.num_rows, self.num_cols);

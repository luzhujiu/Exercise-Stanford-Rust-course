    ...
    for i in 1..=m {
        for j in 1..=n {
            if seq1[i-1] == seq2[j-1] {
                let value = c.get(i-1, j-1).unwrap();
                c.set(i, j, value + 1);
            } else {
                let value1 = c.get(i, j-1).unwrap();
                let value2 = c.get(i-1, j).unwrap();
                c.set(i,j,std::cmp::max(value1, value2));
            }
        }
    }
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("file not open");
    let lines2 = read_file_lines(filename2).expect("file not open");
